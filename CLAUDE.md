...
`tests/` 配下に 19 ファイル、合計 **422 テスト** (420 pass + 1 skip + 1 xfail — skip はショートショット 高 threshold ケース、xfail は Issue #58)：

- `test_smoke.py` — 4件: import / MaterialDB / build_demo_geometry / Cross-WLF 単調性
- `test_solver_1d.py` — 10件: 1Dストリップの解析解 `τ(x) = x(2L−x)/(2S)` との比較。max誤差 <2%、メッシュ細分化で誤差減少を保証 / **行列の構造** — 非対称エントリがゲート行に限られること、固定点を消去した内部ブロックが対称正定値であること（`docs/流動解析の仕組み_想定問答_技術編.md` が顧客に約束している性質なので、散文だけに置かない）、ゲートに繋がらない成分があるとその保証が崩れること（数学の性質なので Issue #58 をどう直しても不変）、**`solve()` がそういう領域に充填時間を捏造しないこと（`xfail(strict=True)`。今の不備を assert で固定すると修正時に消す羽目になり、「旧挙動 or 新挙動」の or で書くと両方で通って信号にならない。strict xfail なら Issue #58 が直った瞬間 XPASS で落ちてマーカー除去を促す）**、ゲート列の消去が解を動かさないこと（消去は厳密）。**mask に穴を開けた形状で組む**のが要点で、mask が全 True だと圧縮行列インデックスと生グリッドインデックスが一致してしまい、両者を取り違えたテストも通る（`Geometry.gates` はグリッド座標、`A` はそうでない）
