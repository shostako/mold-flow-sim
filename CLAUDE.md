...
- **`visualizer_3d.py`** — `render_3d_thickness_map` / `render_3d_fill_time` / `render_3d_pressure`。**インタラクティブな3D表示**用。Plotly の `go.Figure` を返し、Streamlit の `st.plotly_chart` で埋め込む想定。各図は **PL（Z=0）= 半透明の薄グレー床 + 側壁 Mesh3d（天面と同じ物理量カラーマップで着色、`coloraxis="coloraxis"` 共有）+ 天面 Surface** の3トレース構成。1本のカラーバーで天面・側壁を一気に読む設計。**`aspectmode="data"`** で x/y/z すべて mm 等倍（誇張なし）。プレートが薄板に見えるのは実物比率そのもの。物理は 2D Hele-Shaw のまま、表現上の3D化のみ。3レンダラとも `supersample: int = 1`（>=1）を受け、`>1` で **表示専用に**メッシュを補間アップサンプリング（`_supersample_for_render`：`_RenderResult` で細メッシュ Geometry を包んで既存トレースビルダーを流用）して斜め境界・側壁の階段を `1/k` に縮める。補間は **4連結成分ごとの normalized convolution（マスク重み付き補間）**：`label`（4近傍＝solver と同じ連結性）で成分に分け、各成分を `find_objects` のパディング付き bbox に切り出し、その成分のマスク `m` だけで `thk_f = zoom(np.where(m,thk,0))/zoom(m)`、`color_f = zoom(np.where(m,cf,0))/zoom(m)`、`cmask_f = zoom(m) >= 0.5`（すべて `zoom(..., order=1, mode="nearest", grid_mode=True)`）を計算して合成する。これで4つの性質が同時に成立する: ①**背景の隙間を跨がない**（不連続キャビティ間も、U字/リング成分の腕同士が1セルで隣接する内部スロットも、背景は重み0）、②**斜め接触の別キャビティが混ざらない**（角だけで触れる2領域は4連結では別成分＝同じ bilinear カーネルを共有しない。solver の4連結と整合）、③**外殻NaN安全**（充填時間/圧力は cavity 外が `np.nan`（solver 初期化）で `NaN·0==NaN` が `zoom` で境界・側壁色に伝播し着色が消えるため、numerator は `np.where(m, field, 0)` で作る — `field·m` は不可）、④**コスト有界**（`find_objects` で全 bbox を一発取得、`comp_labels==comp` の比較もクロップ内だけ＝`O(総キャビティ面積·k²)`、千個スペックルでも線形）。これら（①境界ゼロにじみ→②成分間にじみ→④性能→①連結成分内スロット→③外殻NaN伝播→②斜め接触混合）は Codex が8ラウンドで段階指摘した系統で、成分別 normalized convolution が一掃した。**【既知の制限】** ただし *bilinear の 2×2 カーネルは斜めに隣接する2つの cavity セルを必ず混ぜる* ため、**1つの連結成分が折り返して2つの腕が斜めピンチ点（角だけ）で接する病的形状**では、その角の細セルで両腕の値が混ざる（成分分割では同一成分なので切れない）。完全に断つには測地線（topology-aware）補間が要るが、表示専用の平滑化に対して過剰なので**意図的に未対応**。発生条件は「**画像アップロード由来のラスタ形状 × 斜めピンチ × supersample>1**」に限られ、パラメトリック形状（Film/Direct gate＝斜めピンチ無し）と既定 `k=1` は完全に無影響。3D は元々「表現上の3D化のみ」の近似表示。**`grid_mode=True` 必須**（既定 `grid_mode=False` はセル中心間距離で拡大するため、宣言した細メッシュ（cs/k のセルが全幅 N·cs を張る）と食い違い、補間値・シルエットが約 1/4 ネイティブセルずれる — Codex P2 指摘）。各ネイティブゲートcellは `k×k` ブロック全体に展開し、平均重心を偶奇問わず元のゲート中心に厳密一致させて表示原点をゲートに固定する（単一オフセットcellだと偶数 k で半セルずれる — Codex P2 指摘）。さらに各ゲートcellの**カラー場の値**をその `k×k` ブロックに restamp する（bilinear が単一セルゲートの `pressure_norm==1` を周囲と平均して薄め、偶数 k では細セル中心がゲート中心に乗らず「1 at gate」がカラーバー最大に出ない問題 — Codex P2 指摘 — を防ぐ）。**solver・geometry・解析時間は不変**（`k=1` は完全な no-op で既存挙動と一致）。点数は `k²` 倍に増え、**毎 rerun で3図すべてを再生成する**（Streamlit は畳んだ expander／非選択タブの中身も実行する）ので操作全体が重くなる。よって **UI 既定は `1`（ネイティブ）**、3D を見るときだけスライダー「描画の細かさ (supersample)」(1〜3, `key="ss_3d"`) を上げる運用。3レンダラに渡す（3D は UI 専用機能なので CLI `run_demo.py` には無い）。
...
`tests/` 配下に 10 ファイル、合計 **232 テスト** (231 pass + 1 skip — short-shot 高 threshold ケース)：
...
- `test_visualizer_3d.py` — 25件: PL extrusion anatomy（PL床 + 天面 + 側壁 Mesh3d）、外殻NaN処理（床は0/天面は厚み）、ゲート中心軸、側壁が PL〜天面を覆う、`aspectmode='data'` で等倍、側壁が天面と coloraxis 共有 + intensity を物理量から継承、**表示supersample**（`supersample=1` で native 解像度維持、`=2` で床/天面/surfacecolor が `(ny*2,nx*2)` に細密化＋壁mesh有効＋data-aspect保持、シルエット保存＋外殻NaN＋床=0維持、mm スパンは半セル以内で不変＋壁三角形数が増加、**ゲート原点が偶奇 k 問わず厳密保存**、**grid_mode=True で線形ランプが細メッシュ中心に整合**=値ズレ回帰、**ゲートcell値が細メッシュでも厳密保持**（単一セルゲートの圧力=1がブロックに残り max が 1 に届く）=圧力「1 at gate」回帰、**不連続キャビティ間で値がにじまない**（`[A . B]` で各成分が定数値を保つ）、**連結成分内の1セルスロットを跨いでもにじまない**（U字で片腕の値が反対腕に出ない）、**斜め接触の4連結別キャビティが混ざらない**（`[[A,.],[.,B]]` で各々の値を保つ）=成分別 normalized conv の隙間/対角非跨ぎ、**多数スペックルが各々自分の値で保持される**、**充填時間/圧力の外殻NaNがサニタイズされ細メッシュ側壁色が有限**=NaN伝播回帰）
