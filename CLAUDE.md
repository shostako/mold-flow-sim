...
- バリデーション: `outer_wall_line` と `sub_gates` は排他で必ず片方、扇形ではトップレベル `island` 不可、`tip_t > land.length`、各線の t 増加・w ≥ 0、先端で外壁 > 内壁、島は角 ≤ ランプ角・`end_dist > land.length`・`[land.length, end_dist]` で外側線 > 内側線、ランナーは幅・深さ正・2 点以上・零長セグメント無し・t, w ≥ 0。未知キーはパス付きで拒否（`sub_gates[0].island` の `weld` は未対応で拒否）。
...
`tests/` 配下に 28 ファイル、合計 **559 テスト** (558 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_geometry_twin_fan.py` — 33件: **`sub_gates` + `runner` 拡張**。round-trip（`to_json` と `dataclasses.asdict` 経由の両方）/ `outer_wall_line` と `sub_gates` の排他・トップレベル島の拒否・扇の tip/壁/島/ランナーの各バリデーション（パス付き）/ `t_max` `w_max` がランナー到達を含む・ランナー越しのはみ出し拒否 / **ランド全幅と中央の鋼材**（扇の中間 t で内壁の内側と外壁の外側が mask 外、先端の先も無い）/ **扇の面積の閉形式**（台形＋ランド、±2%）/ 扇の深さがランド＋ランプ（先端手前でキャップ未到達）/ 扇の島が2線に挟まれた浅い帯で終端の先はランプに戻る / **ランナーのカプセル体積**（鋼材上に置いた帯の体積増 = `depth·(w·L + π(w/2)²)` ×2、±3%）/ **ランナー深さは床であって上書きでない**（帯の外は不変、扇の中は max、鋼材上は帯の深さそのもの）/ **ランナーが可解性そのもの**（無しだとソルバが未到達で拒否、有りで完走）/ ゲートセルがバルブ位置のランナー内 / 井戸の合成 / レガシー demo spec が新フィールド既定で bit 不変
