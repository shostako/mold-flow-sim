...
- **`visualizer_3d.py`** — `render_3d_thickness_map` / `render_3d_fill_time` / `render_3d_pressure`。**インタラクティブな3D表示**用。Plotly の `go.Figure` を返し、Streamlit の `st.plotly_chart` で埋め込む想定。各図は **PL（Z=0）= 半透明の薄グレー床 + 側壁 Mesh3d（天面と同じ物理量カラーマップで着色、`coloraxis="coloraxis"` 共有）+ 天面 Surface** の3トレース構成。1本のカラーバーで天面・側壁を一気に読む設計。**`aspectmode="data"`** で x/y/z すべて mm 等倍（誇張なし）。プレートが薄板に見えるのは実物比率そのもの。物理は 2D Hele-Shaw のまま、表現上の3D化のみ。3レンダラとも `supersample: int = 1`（>=1）を受け、`>1` で **表示専用に**メッシュを補間アップサンプリング（`_supersample_for_render`：`_RenderResult` で細メッシュ Geometry を包んで既存トレースビルダーを流用）して斜め境界・側壁の階段を `1/k` に縮める。補間は **normalized convolution（マスク重み付き補間）**：`thk_f = zoom(np.where(mask,thk,0))/zoom(mask)`、`color_f = zoom(np.where(mask,cf,0))/zoom(mask)`、`mask_f = zoom(mask) >= 0.5`、すべて `zoom(..., order=1, mode="nearest", grid_mode=True)`。背景セルの重みが常に 0 になるので、**どんな背景の隙間も跨いで混ざらない**（不連続キャビティ間も、1つの連結成分(U字/リング)の腕同士が1セルで隣接する内部スロットも）し、**境界が外側のゼロに引っ張られてにじむこともない**（境界の細セルはその cavity セルの値そのもの）。**外側のサニタイズは `np.where(mask, field, 0)` で行う（`field·mask` は不可）**：充填時間/圧力は cavity 外が `np.nan`（solver 初期化）で `NaN·0 == NaN` が残り `zoom` で境界・側壁の色に NaN が伝播して着色が消える — Codex P1 指摘。埋め・連結成分処理・bbox は不要で、`zoom` 3回＝`O(grid·k²)`（多数スペックルの閾値画像でも線形・低コスト）。この方式に至るまで Codex が段階的に指摘した系統（境界ゼロにじみ→グローバル最近傍埋めの成分間にじみ→`label` 成分別＋bbox＋`find_objects` の性能→連結成分内スロットにじみ→外殻 NaN 伝播）を normalized convolution＋サニタイズが一掃した。**`grid_mode=True` 必須**（既定 `grid_mode=False` はセル中心間距離で拡大するため、宣言した細メッシュ（cs/k のセルが全幅 N·cs を張る）と食い違い、補間値・シルエットが約 1/4 ネイティブセルずれる — Codex P2 指摘）。各ネイティブゲートcellは `k×k` ブロック全体に展開し、平均重心を偶奇問わず元のゲート中心に厳密一致させて表示原点をゲートに固定する（単一オフセットcellだと偶数 k で半セルずれる — Codex P2 指摘）。さらに各ゲートcellの**カラー場の値**をその `k×k` ブロックに restamp する（bilinear が単一セルゲートの `pressure_norm==1` を周囲と平均して薄め、偶数 k では細セル中心がゲート中心に乗らず「1 at gate」がカラーバー最大に出ない問題 — Codex P2 指摘 — を防ぐ）。**solver・geometry・解析時間は不変**（`k=1` は完全な no-op で既存挙動と一致）。点数は `k²` 倍に増え、**毎 rerun で3図すべてを再生成する**（Streamlit は畳んだ expander／非選択タブの中身も実行する）ので操作全体が重くなる。よって **UI 既定は `1`（ネイティブ）**、3D を見るときだけスライダー「描画の細かさ (supersample)」(1〜3, `key="ss_3d"`) を上げる運用。3レンダラに渡す（3D は UI 専用機能なので CLI `run_demo.py` には無い）。
...
`tests/` 配下に 10 ファイル、合計 **231 テスト** (230 pass + 1 skip — short-shot 高 threshold ケース)：
