...
`tests/` 配下に 32 ファイル、合計 **643 テスト** (642 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_film_gate8_ui.py` — 8件 (AppTest): 既定スライダーが T字 spec と一致（`runner` 無し・`sub_gates` 2 個・ランプ角 58.78）/ 既定形状が spec 直読みと 0.5 mm で mask・肉厚・ゲート bit 一致 / **ポケットが T 字**（棒: ランド行 0.35・ランプ行・床 2.0 が w=±149 まで、t=4.25 は鋼材。縦ランナー: t=4.25 と 10.25 で |w|=2.25 が 2.0・2.75 は鋼材＝付け根で全幅。バルブ位置は井戸 4.5）/ 井戸 OFF でも縦ランナーがバルブ＋半径まで届き t_max がそこで終わる / **導出量がスライダーに追従**（横ランナー深さ 2.5・ランプ長 2 → 47.07°、奥端 6・縦ランナー幅 8・バルブ 24 → tip_t 25.5）/ 奥端の下限がランプ長に、バルブの下限が奥端＋半径に追従 / **バルブより奥に置いた井戸に縦ランナーが届く**（井戸 40〜50・バルブ 10 で tip_t = 44.5、解析が完走 — Codex P2 on PR #83）/ FG8 → FG1 でスライダー値が漏れない
