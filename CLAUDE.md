...
`tests/` 配下に 29 ファイル、合計 **576 テスト** (575 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_film_gate5_ui.py` — 6件 (AppTest): 既定スライダーが組む spec が L字ランナー検討案の spec と一致（ランナーが3点の L字ポリライン）/ 既定形状が spec 直読みと mask・肉厚・ゲート bit 一致 / L字経路の角 `(バルブ t, 中心半幅)` がバルブ・扇先端・中心半幅の各スライダーに追従 / **バルブ＝扇先端 t で角が終端と重なっても零長セグメントを出さない**（連続重複点を落として2点経路、ビルド完走）/ 中心半幅の端でランナー幅上限（プレート外へのはみ出し防止）が L字経路でも守られる / FG5 → FG4 でスライダー値が漏れない
