...
- **`visualizer_3d.py`** — `render_3d_thickness_map` / `render_3d_fill_time` / `render_3d_pressure`。**インタラクティブな3D表示**用。Plotly の `go.Figure` を返し、Streamlit の `st.plotly_chart` で埋め込む想定。各図は **PL（Z=0）= 半透明の薄グレー床 + 側壁（vertical quad）+ 天面（Z=h、物理量で着色）** の3トレース構成で、**3トレースとも cavity セルだけを張る疎な `go.Mesh3d`**（天面・床は `coloraxis="coloraxis"` 共有、1本のカラーバーで一気に読む）。**天面・床は元々フルグリッドの `go.Surface` だったが**、Surface は cavity 外も NaN で全格子持つため精細化で点数が `k²` 爆発し、ブラウザの WebGL 回転が重くなる。`_cavity_surface_mesh`（cavity セル中心を頂点に、4隣接セルが揃う quad だけ三角形化＝ベクトル化済み）で**cavity だけを張る疎メッシュ**に置換し、NaN パディングゼロ＋Surface より軽い Mesh3d 種別にすることで回転負荷を大幅減（Film gate で頂点約35%減＋トレース種別の効率差）。天面=各 cavity セルの厚みを Z に、物理量場を per-vertex intensity に持つ。**`aspectmode="data"`** で x/y/z すべて mm 等倍（誇張なし）。プレートが薄板に見えるのは実物比率そのもの。物理は 2D Hele-Shaw のまま、表現上の3D化のみ。**表示専用の解析的精細化**：3レンダラは `result` の `.geometry` と物理量場しか読まないので、`refine_for_display(result, geom_fine)` で「`geometry`=細メッシュ・場=補間済み」の軽量ビュー（`_DisplayResult`）に差し替えるだけで、レンダラ・トレースビルダーを一切改造せず細メッシュ描画になる。`geom_fine` は**同じパラメトリック cfg を `cell_size_mm / k` で再ラスタ化**したもの（`build_fine_geometry(cfg, k)`、解析は回さない＝ソルバー不実行・ビルドのみ）。外形シルエットと厚みは `geom_fine` から直接（解析値・補間ゼロ）。fill_time / pressure の**解いた場だけ** `_interp_field_to_fine` で粗メッシュ→細メッシュに補間（物理pad座標で対応付け＝半セルオフセット無し、NaN は `distance_transform_edt` で最近傍埋めしてから bilinear、最後に `geom_fine.mask`（解析的fine外形）で再NaN）。**これが supersample（PR #39, revert済み）の本質的限界を構造的に回避する**：シルエットを「ラスタ済みマスクの bilinear 拡大」ではなく「数式の再ラスタ」から得るので、斜め境界の階段が本物に消える（bilinearが斜め隣接2セルを必ず混ぜる問題は外形側に存在しない）。補間するのは場の色だけ。コストは plotly の点数が `k²` 倍 ×3図（毎rerun再構築）なので、`fine_refine_factor(coarse_cells, k)` で `FINE_DISPLAY_CELL_CAP`（=100万 cavity セル）を超えない最大 `k` にクランプするガードを置く（解析は走らない＝ソルバーの cell_limit とは別枠、ブラウザ描画が固まるのを防ぐ表示専用予算）。UI 既定は `k=1`（ネイティブ＝完全 no-op）、3D を見るときだけ「3D描画の精細さ」スライダー(1〜3, `key="refine_3d"`)を上げる運用。画像入力は cfg 無し（`geom_cfg=None`）なので精細化無効＝ネイティブ表示にフォールバック。**solver・geometry・解析結果は不変**（表示専用）。
...
`tests/` 配下に 11 ファイル、合計 **226 テスト** (225 pass + 1 skip — short-shot 高 threshold ケース)：
...
- `test_visualizer_3d.py` — 19件: PL extrusion anatomy（**3トレースとも Mesh3d**、天面/床は cavity セル数ぶんの頂点＝疎メッシュ）、天面/床が cavity だけを張る（頂点数=cavity数、天面 Z 有限正・床 Z=0）、頂点がゲート中心座標、側壁が PL〜天面を覆う、`aspectmode='data'` で等倍、側壁と天面が coloraxis 共有 + per-vertex intensity を物理量から継承、**表示専用の解析的精細化**（`_interp_field_to_fine` の細メッシュ形状＆fine外形NaN、定数場保存、外殻NaN非伝播、座標対応の半セルオフセット無し、`build_fine_geometry` の再ラスタ＆cfg=None/k≤1フォールバック、`fine_refine_factor` のクランプ、`refine_for_display` が fine geometry を使う、3レンダラが精細化ビューで天面=fine形状・着色finite/NaNを保つ）
...
- 主な依存：numpy / scipy（ソルバ＋3D表示の解析的精細化で `scipy.ndimage` の `map_coordinates` / `distance_transform_edt` を場の補間に使用）、matplotlib（画像書き出し）、Pillow（画像入力）、streamlit（UI）、**plotly**（3D表示、`visualizer_3d.py` 専用）。**注意**：Streamlit は畳んだ expander の中身も毎 rerun で実行するので、3D図3枚は expander が畳まれていても毎回構築・送出される（「開いた時だけコストが走る」わけではない）。よって「3D描画の精細さ」既定は `k=1`（ネイティブ＝最小コスト・完全 no-op）にしてあり、3Dを見るときだけスライダーを上げる。`k` を上げると plotly の点数が `k²` 倍 ×3図に増え、無関係な UI 操作のたびに再構築されて全体が重くなる（`fine_refine_factor` が `FINE_DISPLAY_CELL_CAP` で上限クランプ）。
