...
- **`visualizer_3d.py`** — `render_3d_thickness_map` / `render_3d_fill_time` / `render_3d_pressure`。**インタラクティブな3D表示**用。Plotly の `go.Figure` を返し、Streamlit の `st.plotly_chart` で埋め込む想定。各図は **PL（Z=0）= 半透明の床 + 厚み h(x,y) 分の側壁 Mesh3d + 天面 Surface（物理量で着色）** の3トレース構成。物理は 2D Hele-Shaw のまま、表現上の3D化（Z軸スケールは視認性のため誇張あり、aspectratio z は0.05〜0.5にクランプ）。
...
- `test_visualizer_3d.py` — 6件: PL extrusion anatomy（PL床 + 天面 + 側壁 Mesh3d）、外殻NaN処理（床は0/天面は厚み）、ゲート中心軸、側壁が PL〜天面を覆う
