...
`tests/` 配下に 25 ファイル、合計 **509 テスト** (508 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_weld_detection.py` — 8件: 一様流で 0 / 対向 2 ゲートで中央行だけ、正面衝突は 1.0 / **穴の後ろに出て、手前（分流）と側面には出ない** / ゲート周囲と壁の外は 0 / 合成 V 字の τ で開き角ランプの値（全開 1.0・平行 0・中点 0.5） / 引数検証 / **角度場の再しきい値化**（緩めると増え、締めても新規セルは出ず、正面セルは両方で 1.0、描画が通る） / 角度場なしの結果は `weld_score` にフォールバック
