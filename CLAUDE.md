...
`tests/` 配下に 28 ファイル、合計 **565 テスト** (564 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_geometry_twin_fan.py` — 39件: **`sub_gates` + `runner` 拡張**。round-trip（`to_json` と `dataclasses.asdict` 経由の両方）/ `outer_wall_line` と `sub_gates` の排他・トップレベル島の拒否・扇の tip/壁/島/ランナーの各バリデーション（パス付き）/ `t_max` `w_max` がランナー到達を含む・ランナー越しのはみ出し拒否 / **ランド全幅と中央の鋼材**（扇の中間 t で内壁の内側と外壁の外側が mask 外、先端の先も無い）/ **扇の面積の閉形式**（台形＋ランド、±2%）/ 扇の深さがランド＋ランプ（先端手前でキャップ未到達）/ 扇の島が2線に挟まれた浅い帯で終端の先はランプに戻る / **ランナーのカプセル体積**（鋼材上に置いた帯の体積増 = `depth·(w·L + π(w/2)²)` ×2、±3%）/ **ランナー深さは床であって上書きでない**（帯の外は不変、扇の中は max、鋼材上は帯の深さそのもの）/ **ランナーが可解性そのもの**（無しだとソルバが未到達で拒否、有りで完走）/ **壁の外挿到達**（外壁が `tip_t` 手前で終わって外向きに傾くとき `w_max` が外挿先を返し、はみ出しを拒否／広いプレートではそこに実セルがある。単一ポケット形の `outer_wall_line` も同じ）/ **片側形の負側到達**（`w=0` 付近のランナーが `min(path.w) − width/2` まで届く。余白不足は拒否、足りていれば負側も実 cavity）/ **扇の幅を全区間で検証**（ランド側で交差し先端で開く形状の拒否、clamp 点の折れ目で潰れる形状の拒否、`_fan_breakpoints` が `{0, tip_t, 各線の clamp 点}` を返すこと）/ ゲートセルがバルブ位置のランナー内 / 井戸の合成 / レガシー demo spec が新フィールド既定で bit 不変
...
- `test_film_gate4_ui.py` — 7件 (AppTest): 既定スライダーが組む spec がミニ扇×2 検討案の spec と一致（扇の壁は先端 t／中心半幅／先端幅から、ランナー経路はバルブと先端から導出）/ 既定形状が spec 直読みと mask・肉厚・ゲートセル一致 / **扇の中間 t でバルブ軸周りが穴、左右の扇が cavity** / 肉盗み・井戸 OFF で `sub_gates[0].island` と `well` が null / **ランナー経路がバルブと先端に追従**（スライダーを動かすと `path` と壁の終点が同時に動く）/ **扇先端幅の上限 = 2·min(中心半幅, 出口半幅 − 中心半幅)**（内壁が w=0 を割ると `validate()` が拒否する値をスライダーが出さない）/ FG4 → FG1 でスライダー値が漏れないこと
