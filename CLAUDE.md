...
`tests/` 配下に 31 ファイル、合計 **624 テスト** (623 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_film_gate6_ui.py` — 5件 (AppTest): 既定スライダーが 0807 改修 spec（帯込み）と一致 / 既定形状が spec 直読みと mask・肉厚・ゲート bit 一致 / **帯が肉厚場に実在**（端 w=148.5 で t=4,5 が 2.5、ランド端と t=2 はランプのまま、w=140.5 の t=3 もランプ、帯無しビルドとの差分セルが全部「ランプ < 2.5」のセルで新値は 2.5）/ 帯 OFF で `edge_channels` が空になり残りは改修ポケットのまま / **既定 ON が Film gate 1 に漏れない**（`f6_ec_on` True → 1 に切替で `f1_ec_on` False、記録も空）
