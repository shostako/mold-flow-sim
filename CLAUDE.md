...
- `test_visualizer_3d.py` — 19件: PL extrusion anatomy（PL床 + 天面 + 側壁 Mesh3d）、外殻NaN処理（床は0/天面は厚み）、ゲート中心軸、側壁が PL〜天面を覆う、`aspectmode='data'` で等倍、側壁が天面と coloraxis 共有 + intensity を物理量から継承、**表示専用の解析的精細化**（`_interp_field_to_fine` の細メッシュ形状＆fine外形NaN、定数場保存、外殻NaN非伝播、座標対応の半セルオフセット無し、`build_fine_geometry` の再ラスタ＆cfg=None/k≤1フォールバック、`fine_refine_factor` のクランプ、`refine_for_display` が fine geometry を使う、3レンダラが精細化ビューで天面=fine形状・着色finite/NaNを保つ）
