...
- **`solver.py`** — `HeleShawSolver` と結果 `FlowResult`。中核アルゴリズムは下記。**スキン層の時計は露光時計**（Issue #61, v0.33.0）: 壁は先端が通過した瞬間から老化し続けるので、セルの求解に当てるスキンは役務期間 `[t_arr, min(T_fill, t_close)]` の時間平均 `(2/3)·s(a)`（`SKIN_SERVICE_MEAN_FACTOR`）。スキンが出会う年齢 `t_c = ((h−h_min)/(2c))²/α` に役務が届いたセルは `t_close = t_arr + t_c` で**封止**する（`short_shot_mask`。充填後に閉じたので充填時間を持つ）。**未充填は到着時刻付きの到達性で決める**: 自分の到着時刻に、開いているセル（封止しない、または `t_close` がまだ先）だけの4近傍経路でゲートと繋がらないセルが `FlowResult.unfillable_mask` に入り、`fill_time_s` / `pressure_norm` は NaN になる（`_unfillable_cells(frozen, t_arr, t_close)`、逆時間の union-find）。`short_shot ∩ unfillable = ∅`。**cavity 全体の解が切断を出したら、充填する部分は「自分を切らない最大の接頭集合」を充填体積の二分探索で求める**（`_largest_consistent_prefix`。順序は全体解の τ、各候補は `_restricted_to` で切り出して無スキンから独立に解く。分解能は `V/DOMAIN_VOLUME_RESOLUTION(256)`＝8候補、ただし最薄セル1個より細かくはしない。仕上げに hi 側候補の切断が lo〜hi の不確定帯に収まっていれば「hi − 切断」を1回解いてセル単位に境界を詰める。切断が帯の下まで及ぶ＝定圧近似の正帰還が折り返した（一滴足すと全体が封止に転ぶ）ケースで、そこは lo が答え）。**なぜ二分探索か**: 死領域を抱えたまま解くと時計がその抵抗で膨張して封止が早まり切りすぎ、切った残りだけを解き直すと抵抗が足りず何も封止しない速い時計になる — どちらもその部品の解ではない（旧 monotone 縮小方式は LGP 0.4mm で「ゲート側の h_core は満杯なのに製品の途中で切れる」絵を出した）。`MAX_DOMAIN_PASSES=64` は暴走止めで、候補ゼロ（cap 0）なら全体解に切断を当てて `domain_converged=False`。絶対時間の基準はその解の τ（`tau_max_flow`）、ベースライン時間も充填する体積で計算する。`short_shot_mask` は最終解で閉じたセル ∪ 最小の切れた候補（hi）が閉じたと見たセル（充填を一滴増やすと閉じる封止。lo 自身の時計では閉じる直前で終わるので lo だけ見ると封止が消える）。時間平均スキンは `(2/3)(h−h_min)/2` を超えないので `h_core ≥ h/3`、封止セルの抵抗増は最大 27 倍 — 旧到着時計では床打ちセルが T_fill を数百倍に膨らませて未収束のまま止まっていた（LGP 0.4mm, c=0.5 で 414 倍→1.1 倍）。
...
`tests/` 配下に 26 ファイル、合計 **516 テスト** (515 pass + 1 skip — skip はショートショット 高 threshold ケース)：
