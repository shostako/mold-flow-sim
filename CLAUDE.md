...
- **扇の島の平底 `floor_depth`（v0.41.0）**: 帯内の深さを `min(ランプ, floor_depth)` にする（角度で育つ帯の代わり）。ランプが浅い所は触らないので台の上流端は「ランプが floor_depth に達する t」。指定時は `angle_deg=0` 必須、`0 < floor_depth ≤ cap_depth`。島の線の「外側 > 内側」検査はラスタと同じクランプ評価（`_edge_w`）— 外挿だとランドより奥から始まる線が負の w で偽の交差になる。
...
`tests/` 配下に 32 ファイル、合計 **654 テスト** (653 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_geometry_twin_fan.py` — 55件: **`sub_gates` + `runner` 拡張**。round-trip（`to_json` と `dataclasses.asdict` 経由の両方）/ `outer_wall_line` と `sub_gates` の排他・トップレベル島の拒否・扇の tip/壁/島/ランナーの各バリデーション（パス付き）/ `t_max` `w_max` がランナー到達を含む・ランナー越しのはみ出し拒否 / **ランド全幅と中央の鋼材**（扇の中間 t で内壁の内側と外壁の外側が mask 外、先端の先も無い）/ **扇の面積の閉形式**（台形＋ランド、±2%）/ 扇の深さがランド＋ランプ（先端手前でキャップ未到達）/ 扇の島が2線に挟まれた浅い帯で終端の先はランプに戻る / **ランナーのカプセル体積**（鋼材上に置いた帯の体積増 = `depth·(w·L + π(w/2)²)` ×2、±3%）/ **ランナー深さは床であって上書きでない**（帯の外は不変、扇の中は max、鋼材上は帯の深さそのもの）/ **ランナーが可解性そのもの**（無しだとソルバが未到達で拒否、有りで完走）/ **壁の外挿到達**（外壁が `tip_t` 手前で終わって外向きに傾くとき `w_max` が外挿先を返し、はみ出しを拒否／広いプレートではそこに実セルがある。単一ポケット形の `outer_wall_line` も同じ）/ **片側形の負側到達**（`w=0` 付近のランナーが `min(path.w) − width/2` まで届く。余白不足は拒否、足りていれば負側も実 cavity）/ **扇の幅を全区間で検証**（ランド側で交差し先端で開く形状の拒否、clamp 点の折れ目で潰れる形状の拒否、`_fan_breakpoints` が `{0, tip_t, 各線の clamp 点}` を返すこと）/ **メッシュより細いランナーの拒否**（幅 1.0/セル 1.0 で帯が点線化＝拒否、セル 0.25 なら通って連結成分1個）/ **折り返しを分断と数えないこと**（軸に触れない帯は全格子上で実際に2片、半平面で数えるので通る）/ ゲートセルがバルブ位置のランナー内 / 井戸の合成 / レガシー demo spec が新フィールド既定で bit 不変 / **扇のエッジチャネル**（外壁帯は外壁に密着し内壁側は不変（独立オラクル: 密サンプル壁線への距離）・inner/outer/両側 = 和集合・帯が扇の外（菱形の鋼材）を復活させない・round-trip 両経路・side/t_range バリデーション・demo spec に無いこと）/ **平底の島 `floor_depth`**（0.8 の台が三角形の中だけ 0.8 でランプが浅い所と外は不変・同じ足跡の角 0 島はランド深さ・片方の扇にしか置かないと他方のランプに負けて消える・round-trip と無指定時の省略・demo spec 不変・バリデーション 4 種（正・≤ cap・角 0 必須）・**ランドより奥から始まる線がクランプ評価で通り、本物の交差は弾かれる**）
