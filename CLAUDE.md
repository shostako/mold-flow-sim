...
`tests/` 配下に 34 ファイル、合計 **729 テスト** (728 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- **壁面冷却モデル**: UI ヘッダー「壁面冷却モデル」のラジオで `なし` / `スキン層` / `層別` の 3 択。**排他選択**により skin と multilayer の同時 ON は構造的に不可能。**UI 既定は『スキン層』**（v0.39.0。v0.29.0〜v0.38 は『なし』だった — 二相ショートショットが『なし』専用だった名残で、v0.37.0 でスキン層が二相の射出相に乗ってから既定を戻す理由が無くなった。『なし』は η 定数で S ∝ h³ になり材料も温度も充填順序に効かない。スキン層で効くのは α だけで T_melt / T_mold は依然効かない — 温度まで効かせるのは層別のみ、二相と併用不可）。層別を選んだときの既定は **N=7 / wall_refined / max_iter=12**（極薄 t<0.5mm 向け）。
  ...
  - **スキン層**: `HeleShawSolver(skin_layer_enabled=True, skin_growth_constant=c_skin, skin_clock_mode=...)`。Stefan/Neumann 1 層モデル。ラジオ「スキン層の時計」（`key="skin_clock"`、既定 `constant_pressure`）で時計の応答を選ぶ。settings.json の `wall_cooling.skin_clock_mode` に記録。
