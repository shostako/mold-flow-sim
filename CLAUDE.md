...
`tests/` 配下に 18 ファイル、合計 **373 テスト** (372 pass + 1 skip — short-shot 高 threshold ケース)：
...
- `test_visualizer_3d.py` — 24件: block anatomy（**3トレースとも flat-top Mesh3d**、天面=各 cavity セル2三角形＝面数 2×cavity・`intensitymode="cell"`）、天面が全 cavity セルを覆う（面数=2×cavity＝侵食ゼロ・天面 Z 有限正・床 Z=0）、**flat-top が対角境界を塞ぐ**（対角バンドで面数=2×cavity）、頂点がゲート中心座標（セル端の隅）、境界壁が PL〜天面を覆う、`aspectmode='data'` で等倍、天面=面ごと/側壁=頂点ごと intensity で coloraxis 共有、**厚み段差が縦の段差として描かれる**（段差プレートで天面 z が両厚みを保持＋PL非接触の段差壁が立つ）、**天面ホバーが場の値を露出**（fill/pressure で per-vertex customdata が読める）、**肉厚の配色が 2D マップと一致し、実際に明→暗で走ること**（Plotly が解決した colorscale の RGB を輝度に落として比較する。名前だけ突き合わせると `Cividis_r` が実は反転していなくても通ってしまう）、**その単調性を ramp 全域で見ること**（両端だけの判定は組込188本中26本に破られる。最悪は `jet_r` で、両端の輝度差 0.070 に対し中間で 0.719 も逆行する＝「明→暗」を名乗るレインボー。肉厚マップは暗さだけで厚みを順序づけられることが要件で、輝度が逆行する所では別の厚みが同じ暗さになり順序が復元できない。判定は stop だけでなく **stop 間を密サンプル**して行う — Plotly は符号化 sRGB で線形補間するが relative luminance は非線形変換を先に掛けるので、区間内の輝度は線形にならず 「stop で単調なら全域で単調」は成立しない。`bluered_r` が実例で、stop 判定を通って区間内で 0.0023 逆行する)、**薄肉端と厚肉端のコントラスト比が 3:1 以上あること**（WCAG 1.4.11。単調なだけなら近黒→黒の ramp も通り、順序は原理的に復元できても地図が読めない。`cividis_r` は 12.56:1）、**基準そのものの判別力**（`Cividis_r` / `Blues` / `Blues_r` / `Jet_r` / `Bluered_r` / `Greys_r` で期待値を固定。定数を差し替える変異注入は名前一致 assert が先に落ちて判定にならないため、基準を直接テスト）。色の計算は `tests/colorimetry.py` に集約してある（`css_rgb` / `relative_luminance`）— **輝度の式を2ファイルに手で書き写していて、両方が同じ間違いをしていた**のが集約の理由。`0.2126R+0.7152G+0.0722B` をガンマ符号化された sRGB にそのまま当てると luma であって relative luminance ではなく、チャンネル間で強度を振り替える配色では符号が食い違う（`rgb(205,78,46)→rgb(242,34,36)` は naive だと暗化、線形化すると明化）。組込188本では判定が一致するので現行バグではないが、手で作った暖色 ramp を評価するときに効いてくる、**colorscale の stop が hex と `rgb(...)` のどちらでも読めること**（Plotly の名前付きスケールは 94個中81個が `rgb(...)` 形式を返し、`matplotlib.colors.to_rgb` はそれを拒否する。Cividis はたまたま hex 側なのでhex 専用パーサでも今は通るが、別の配色候補を評価した瞬間に「配色の判定」でなく無関係に見える ValueError で死ぬ）
...
- 主な依存：numpy / scipy（ソルバ）、matplotlib（画像書き出し）、Pillow（画像入力）、streamlit（UI）、**plotly**（3D表示、`visualizer_3d.py` 専用）。**注意**：Streamlit は畳んだ expander の中身も毎 rerun で実行するので、3D図3枚は expander が畳まれていても毎回構築・送出される（「開いた時だけコストが走る」わけではない）。3D は疎 Mesh3d でネイティブ解像度描画する（点数は cavity セル数ぶん）。3D の回転負荷を下げるためフルグリッド Surface を使わない設計。
