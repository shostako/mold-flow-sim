...
`tests/` 配下に 21 ファイル、合計 **464 テスト** (463 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
- `test_two_phase.py` — 21件: 不変条件（Ω₁ ⊆ Ω₂ / **最終形状が計量体積をタイ群粒度で保持**（超過ゼロ）/ 計量のネスト単調性 / フルショットで完全充填 / ゲートは常に Ω₁ / プール外の到着時刻 NaN）/ **ストリップ解析解**（前線位置 n1 = V/(dx²·h_open)、圧縮後 n2 = n1·h_open/h_fin の純算術 / 到着時計の体積線形性 / 前進の連続性 / progress の単調性と終端1）/ 退化と拒否（ICM OFF・stroke 0 で前進なし / factor モード受理 / スキン層拒否 / 非正体積拒否 / ゲート無し拒否）/ metadata 契約 / **タイ原子性**（2×n 双子ストリップ: 5.5列分の計量が11半セルでなく5全列で止まる）/ **等圧ソース契約**（tau2 が Ω₁ 全セルで 0、外で正）/ 描画（部分/完全ショット両方 / **色のピクセル数比**が領域セル数比に整合 — 凡例パッチが色の存在を常に供給するので「色がある」だけの検証は空）
- `test_two_phase_ui.py` — 3件 (AppTest): 既定 OFF で何も走らない / ON + ICM で map 生成・settings.json 記録・ZIP に `two_phase_short_shot.png` と `two_phase_metadata.json` 同梱 / 壁面冷却モデル選択時は警告してスキップ。**expander 内の caption は AppTest の平面リストに出ない**ので、描画は session_state のパスとファイル実体で検証する
...
- **二相ショートショット**: UI はサイドバー「ショートショット（計量制限）」expander の checkbox（`key="two_phase_on"`）＋計量体積 number_input（`key="two_phase_shot_volume"`、cm³）。壁面冷却モデルが『なし』以外だと警告してスキップ（モデルの用途定義: 計量律速は凍結を含まない）。結果ペインに専用 expander（マップ + 計量体積 / 射出終了時充填率 / 圧縮後充填率）、ZIP に `two_phase_short_shot.png` / `two_phase_metadata.json`、settings.json に `two_phase_short_shot` セクション。CLI は `_solve_and_export(two_phase_shot_volume_cm3=...)`（skin / multilayer と排他、`FilmGate_PP_two_phase_short` が参照ケース）。
