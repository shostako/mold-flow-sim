...
| `Film gate 9 (扇状/9月末試作)` | `f9` | True | `hamoko_gate_furiwake_runner_20260914`（2026/09/14 図面「9月14日ランナ案」） | 0807 改修のポケットから**外壁線だけ**を動かした形（縁部深彫り・水平部は既定 OFF）。現状ライン (3〜5, 149)→(23.28, 4.48) は 8°、新ライン（図面の二点鎖線）は (15.736, 149)→同じ終点で 3°。図面の「5°」は両線のなす角、「(15.736)」は端での新ライン位置。深さ 2.5 到達線 t=12.11 が外壁で止まらず両端まで届き、ランプの奥がほぼ全幅の深さ 2.5 の帯（端で長さ 3.6、中央で約 11）＝扇でなく横ランナーでフィルム全幅を養う。外壁開始 15.736 はスライダーの step 0.1 に乗らないが既定値はそのまま記録される（FG2 の 23.28 と同じ）。断面図の井戸は 0703 図の 60° のまま描かれているが、平面詳細（17/20/23・幅 6・R3）が 0807 改修の 71.6° なのでそちらを採用。等温・スキン無しの充填では製品内の等時線が改修形よりほぼ水平になる |
