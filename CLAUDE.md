...
- `test_film_gate8_ui.py` — 8件 (AppTest): 既定スライダーが T字 spec と一致（`runner` 無し・`sub_gates` 2 個・ランプ角 58.78）/ 既定形状が spec 直読みと 0.5 mm で mask・肉厚・ゲート bit 一致 / **ポケットが T 字**（棒: ランド行 0.35・ランプ行・床 2.0 が w=±149 まで、t=4.25 は鋼材。ステム: t=4.25 と 10.25 で |w|=2.25 が 2.0・2.75 は鋼材＝付け根で全幅。バルブ位置は井戸 4.5）/ 井戸 OFF でもステムがバルブ＋半径まで届き t_max がそこで終わる / **導出量がスライダーに追従**（棒深さ 2.5・ランプ長 2 → 47.07°、奥端 6・ステム幅 8・バルブ 24 → tip_t 25.5）/ 奥端の下限がランプ長に、バルブの下限が奥端＋半径に追従 / **バルブより奥に置いた井戸にステムが届く**（井戸 40〜50・バルブ 10 で tip_t = 44.5、解析が完走 — Codex P2 on PR #83）/ FG8 → FG1 でスライダー値が漏れない
...
| `Film gate 8 (T字/全幅ランナー棒)` | `f8` | True | `hamoko_gate_T_20260910`（2026/09/10 会長の手書きスケッチ「T字ゲート」、Monday 図面化） | T 字。全幅ランド 1×0.35 → ランプ 1 mm で深さ ② 2.0 へ（58.78°）→ 全幅ランナー棒 t=2〜4（Tの横棒）→ 中央ステム 幅 ⑤ 5 × 深さ 2（Tの縦棒）→ 井戸・Φ3 バルブは 0703 と同一（t=21.5）。棒とステムはどちらも `sub_gates` の矩形（内壁 w=0、外壁 w=149 / 2.5、tip_t = 4 / バルブ t＋半径）で `runner` 無し — カプセルだと付け根が丸くなるか（丸端をランドに食い込ませない起点まで下げると）欠ける。原図の台形断面（開口 2.5・床 1.5）は手前壁をランプ、奥壁を t=4 の垂直壁に置換。**既定メッシュ 0.5**（t_max=27.5 で 1.0 mm はセル中心が整数 t に乗り、境界包含でランド・ランプが1行ずつ太って体積 +20%）。丸囲み ⑤ 2.5 1.5 ② は原図で「流入バランスを見ながら深く・広げる」調整寸法 |
