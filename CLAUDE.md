...
`tests/` 配下に 28 ファイル、合計 **570 テスト** (569 pass + 1 skip — skip はショートショット 高 threshold ケース)：
...
`app.py` のサイドバー入力は6系統（`Film gate 1` / `Film gate 2` / `Film gate 3` / `Film gate 4` / `Direct gate` / `Profile gate`）。**Film gate 1 は v0.28.0、片側版（現 Film gate 3）は v0.29.0、Film gate 2 は v0.30.0 で差し替えた** — 旧「台形＋半円ランナー＋▽肉盗み」（`FilmGateConfig` / `build_film_gate_geometry`）と旧「ゲート位置可変」（`FilmGate2Config` / `build_film_gate2_geometry`）は **CLI / ライブラリ専用**に降格し、UI の Film gate 1 / 2 / 3 はスライダーで `GateProfileSpec` を組み立てて Profile gate と同じ `build_profile_gate_geometry` に流す。1〜3 は共通ヘルパー（`_profile_gate_sidebar(tag, symmetric, defaults)` → `_profile_gate_from_inputs` → `_build_film_gate`）で、違いは `symmetric` と `_ProfileGateDefaults` だけ。**Film gate 4（v0.34.0）は別のサイドバー** `_twin_fan_sidebar(tag, _TwinFanDefaults)` → `_twin_fan_from_inputs` で、`sub_gates` + `runner` 形の spec を組む（後述）。製品形状・井戸のブロックは `_plate_shape_inputs` / `_well_inputs` / `_well_from_inputs` として全 Film gate で共有。`_FILM_GATES` dict（ラベル → `_FilmGate(tag, record_name, sidebar, assemble)`）がラジオの選択肢・サイドバー分岐・`build_geometry` の分岐の単一ソースで、`_build_film_gate(entry, v, source)` は `entry.assemble` を呼ぶ。
