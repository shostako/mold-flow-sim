...
- `τ` は擬似到達時間場。絶対時間化は**体積CDF写像**（v0.25.0, Issue #52）: セルの充填時刻 = 「そのセル以下の τ を持つセル群の体積」/ Q。**`injection_profile` を渡すと Q は一つの数でなくなり**（v0.42.0, Issue #88）、体積は `InjectionProfile.time_at_volume_mm3` を通って時刻になる（`injection_volume_flow_cm3s` に優先）。`T_fill` への正規化 `(t_prof/t_prof(total))·T_fill` は残るので、最後のセルが `T_fill` に乗る不変条件・スキン膨張・ICM 短縮との合成は変わらない — 変わるのは途中の形だけ。プロファイル無しの経路は式もそのままで既存結果と bit 一致する。`_baseline_fill_time` と two_phase の `T_inj` も同じ写像を通る（`_injection_time_for_volume_s`）。定率射出なら先端は体積線形で進むので、これが物理そのもの（旧 `(τ/τ_max)·T_fill` 線形写像は健全な1Dストリップでも中央を 0.75T と誤報していた — 正しくは 0.5T）。外れ値1セルは自分の体積分しか他セルの時刻を動かせない。同値 τ はグループ末尾の時刻を共有。
...
`tests/` 配下に 34 ファイル、合計 **730 テスト** (729 pass + 1 skip — skip はショートショット 高 threshold ケース)：
