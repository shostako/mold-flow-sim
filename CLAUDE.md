...
`tests/` 配下に 11 ファイル、合計 **216 テスト** (215 pass + 1 skip — short-shot 高 threshold ケース)：
...
- `test_visualizer_3d.py` — 9件: PL extrusion anatomy（**3トレースとも Mesh3d**、天面/床は cavity セル数ぶんの頂点＝疎メッシュ）、天面/床が cavity だけを張る（頂点数=cavity数、天面 Z 有限正・床 Z=0）、**疎メッシュが対角境界を塞ぐ**（全2×2ブロックが3セルの対角バンドで全頂点が三角形に参照される＝3セル角の三角形化）、頂点がゲート中心座標、側壁が PL〜天面を覆う、`aspectmode='data'` で等倍、側壁と天面が coloraxis 共有 + per-vertex intensity を物理量から継承
