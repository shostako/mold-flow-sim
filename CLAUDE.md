...
`tests/` 配下に 17 ファイル、合計 **338 テスト** (337 pass + 1 skip — short-shot 高 threshold ケース)：
...
- `test_geometry_profile_gate.py` — 34件: JSON I/O（round-trip、パス付きエラー、未知キー拒否、非オブジェクトのセクション拒否）/ バリデーション / シルエット（ランド・ランプ式・cap 底打ち・アイランド・外壁・対称性 flip・非対称）/ 井戸（フロア深さ・max 合成・体積差分を放射積分と照合）/ **閉形式の体積検算**（直壁最小スペック ±3%）/ グリッドはみ出し拒否 / compression_mask / プレート2層 / solver 統合 / **溶接ダム（`island.weld`）**（帯内は一定深さ・帯外と島外は不変・体積減を求積と照合・未指定時は旧挙動と bit-identical・round-trip・バリデーション）
