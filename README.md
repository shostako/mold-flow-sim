...
- 充填先端アニメーション（GIF）、相対圧力マップ、ウェルド／メルドライン（合流角ベース）、エアトラップ候補
- パラメータ：樹脂（PP / PP_T10 / PP_T20 / PP_T30 / ABS / PC / PA66 / PMMA）、樹脂温度、金型温度、射出速度、射出体積流量
- 形状入力：パラメトリック形状（Film gate 1 / 2 = 肉厚調整ゲートの扇状版（肉盗み 1 / 2）、Film gate 3 = 片側版、Film gate 4 = 振り分け／ミニ扇×2（中央を完全肉盗みにしてランナーで左右のミニ扇へ振り分ける検討案）、Film gate 5 = その L字ランナー版（真横に伸ばして各扇の先端に下から中央接続し左右を均等化）、Film gate 6 / 7 = 扇状版の縁部深彫り（外壁沿いの深い帯で樹脂を両端へ先行させる 2026/08/07 改修案 ／ 2026/05/15 提案）, Direct gate）、または図面から起こした JSON スペック（Profile gate）
...
- **壁面冷却モデル（3 択）**:
  - スキン層 1 層モデル — Stefan/Neumann で壁凍結フロント `s(t) = c_skin·√(αt)` を取り込み、コア層 `h_core = h - 2s` だけが流れる。時計は露光時計（壁は先端通過後も老化し続ける）、封止セルとその先の未充填を予測
...
2. **形状入力** — パラメトリックゲート 5 種（Film gate 1 / 2 / 3 / 4 / Direct gate）と
   図面由来の JSON スペック（Profile gate）。中立面メッシュは未着手のまま残る
3. **過渡熱と固化層** — スキン層 1 層モデルと層別 N 層モデル、剪断発熱 段階1
4. **ICM の 2 相化（部分）** — 二相ショートショットモデル（v0.27.0）。射出相
   （型開きギャップで計量体積まで充填）→ 圧縮相（溶融プールを等圧ソースにして
   最終肉厚で前進、体積保存）を線形求解 2 回で解き、計量制限の短絡形状と圧縮の
   前進順序を出す。**通常の充填時間・圧力マップは今も等価モデル**（`h` を膨らませて
   `T_fill` を短縮）のまま、圧縮相は順序のみで物理時間を持たず、圧縮中の凍結と
   射出／圧縮のオーバーラップは扱わない

残り:

5. **流動場から速度場へのフィードバック** — 層別モードは既に反復ごとに
...
6. **数値の地盤強化** — 反復法 (CG / AMG)、メッシュ収束テスト
   （行列組立のベクトル化は v0.33.0 で済み: 10k セルで 180 ms → 9 ms）
7. **入出力実用化** — VTK エクスポート、材料 DB の出典明記・拡張
8. **ICM の正直化（続き）** — 二相モデルを本線に接続する。圧縮相に型締め速度
   由来の実時間を与え、充填時間・圧力の主出力を等価モデルから置き換える。
   圧縮中の凍結（壁面冷却モデルとの併用）はその先
9. **剪断発熱 段階2** — エネルギー方程式の厚み方向 1D FDM 陰解法。
...
- 絶対時間スケーリング：体積CDF写像 `fill_time(x,y) = T_fill · V(τ' ≤ τ(x,y)) / V_solved`（定率射出で先端は体積線形に進む。τ 順に体積を積算した到達時刻。`V_solved` は充填可能なセルの体積で、封止で切られた未充填セルは含まない）
- `T_fill = V_solved / Q` は定率射出・圧縮なし（壁面冷却なし、またはスキン層の速度制御時計）のとき。射出圧縮の等価モデルは短縮し、スキン層の圧力一定時計と層別の熱結合は体積重み付き τ の比で再スケールする
- 面コンダクタンスは隣接セルの調和平均
...
`τ ↔ T_k ↔ η_k ↔ S_total` を fixed-point で結合、体積重み付き平均 τ の比（反復後 / baseline）で `T_fill` をスケール。中央層温度が固化しきい値を切ったセルをショートショットにマーク。`MultilayerHeleShawSolver(num_layers=5, layer_distribution="wall_refined", thermal_coupling=True)` から呼ぶ。`num_layers=1` + `thermal_coupling=False` で既存 `HeleShawSolver` と数値同一 (テスト担保)。
