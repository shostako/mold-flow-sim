...
- 形状入力：パラメトリック形状（Film gate 1 / Film gate 2 / Direct gate）、または図面から起こした JSON スペック（Profile gate）
...
4. **局所粘度反復** — 現状は代表剪断速度で Cross-WLF を 1 回評価するだけ
   （層別モードでも層ごとの代表値であって、局所 `V` の反復ではない）
