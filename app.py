...
with st.expander("📐 使用している方程式と適用範囲"):
    ...
    st.markdown(
        ...
        "セルの充填時刻 ＝ 総充填時間 × （そのセル以下の $\\tau$ を持つセル群の体積 / キャビティ体積）。"
        "定率射出なら先端は注入体積に比例して進むので、順序を体積で刻むのが物理そのもの\n"
        r"$t_{\text{fill}}(\mathbf{x}) = T_{\text{fill}} \cdot \dfrac{V(\tau \le \tau(\mathbf{x}))}{V_{\text{cavity}}}$"
        "\n- $T_{\\text{fill}} = V_{\\text{cavity}} / Q$ は**定率射出・圧縮なし**のとき。"
        "射出圧縮 ON なら §7 の等価モデルで短縮され、スキン層の「圧力一定」時計なら抵抗比で膨張する"
        "（そのとき $t_{\\text{fill}}$ は $V/Q$ で割った値ではない）\n"
