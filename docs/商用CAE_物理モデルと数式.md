...
金型側の熱マネジメントを解きます。① のエネルギー保存が扱う「樹脂内部の熱」とは別に、金型母材の熱伝導・冷却水路の熱伝達・サイクル平均温度場を求め、冷却時間やサイクルタイム、反り・収縮の起点となる温度の偏りを評価します。本ツールは未対応（金型温度は所与の境界条件として入力に使い、樹脂側の粘度・固化・温度分布に反映しますが、金型側を解いて温度場そのものを求めることはしません。壁面冷却モデルも樹脂側の固化であって金型解析ではありません）。
