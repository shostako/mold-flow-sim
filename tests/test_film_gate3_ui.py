"""AppTest wiring checks for the parametric Film gate 3 (片側/二倍流動長).
...
FILM_GATE3_LABEL = "Film gate 3 (片側/二倍流動長)"
...
def _recorded_spec(at: AppTest) -> dict:
    ...
    assert geom["input"] == FILM_GATE3_LABEL
    return geom["gate_profile"]


def test_default_sliders_reproduce_the_2bai_spec(film_gate3_run):
    rec = _recorded_spec(film_gate3_run)
...
def test_default_geometry_matches_the_2bai_spec_built_directly(film_gate3_run):
    geom = film_gate3_run.session_state["mfs_geom"]
...
def test_the_gate_sits_at_the_valve_side_edge(film_gate3_run):
...
def test_film_gate_sliders_do_not_leak_across_the_two_inputs():
    ...
    so a Film gate 3 value would survive the switch to Film gate 1 and
...
def test_a_one_sided_orifice_outside_the_pocket_is_rejected_not_snapped(monkeypatch):
    """FG3 twin of the FG1 guard test: the symmetric=False branch of the
