...
# -------------------------- balancer dynamic base position --------------------------
...
def _dynbase_cfg(
    ...
    *,
    h_runner: float = 2.5,
    plate_lower_thk: float = 0.35,
    runner_depth: float = 20.0,
    runner_flat_depth: float = 8.0,
    base_offset: float | None = None,  # default = runner_depth (long edge)
) -> FilmGateConfig:
    """Single-stage balancer config with explicit residual thickness h_k."""
    return FilmGateConfig(
        plate_w_mm=300.0,
        plate_h_mm=50.0,
        plate_thk_mm=plate_lower_thk,
        runner_long_mm=250.0,
        runner_short_diameter_mm=10.0,
        runner_depth_mm=runner_depth,
        runner_thk_mm=h_runner,
        runner_flat_depth_mm=runner_flat_depth,
        runner_slope_depth_mm=runner_depth - runner_flat_depth,
        valve_gate_diameter_mm=3.0,
        gate_width_mm=250.0,
        cell_size_mm=0.5,
        balancer_enabled=True,
        balancer_height_mm=runner_depth - 2.0,  # apex 2 mm above gate disk
        balancer_base_distance_from_gate_mm=base_offset
        if base_offset is not None
        else runner_depth,
        balancer_base_widths_mm=(60.0,),
        balancer_thicknesses_mm=(h_k,),
        plate_split_height_mm=20.0,
        plate_lower_thk_mm=plate_lower_thk,
        plate_upper_thk_mm=0.50,
    )


def _stage_top_y_mm(g, h_k: float) -> float:
    """Find the maximum y where the balancer paint (= cells with thickness h_k)
    appears. Returns NaN when nothing was painted."""
    iy_idx, _ = np.indices(g.mask.shape)
    yy = (iy_idx + 0.5) * g.cell_size_mm
    in_stage = g.mask & np.isclose(g.thickness_mm, h_k)
    if not in_stage.any():
        return float("nan")
    return float(yy[in_stage].max())


def test_balancer_h_below_plate_thk_extends_to_long_edge() -> None:
    """h_k ≤ plate_lower_thk should keep the legacy behaviour: the
    balancer extends all the way to the plate long edge."""
    cfg = _dynbase_cfg(h_k=0.30, plate_lower_thk=0.35)
    g = build_film_gate_geometry(cfg)
    pad = cfg.pad_mm
    y_long = pad + cfg.runner_short_diameter_mm / 2 + cfg.runner_depth_mm
    y_top = _stage_top_y_mm(g, 0.30)
    # Stage paint should reach within one cell of the long edge
    assert abs(y_top - y_long) <= 2 * cfg.cell_size_mm, (
        f"expected paint at y_long={y_long:.2f}, got max y={y_top:.2f}"
    )


def test_balancer_h_above_plate_thk_stops_in_slope_zone() -> None:
    """h_k > plate_lower_thk should stop where the bare runner thickness
    h(y) equals h_k, i.e. inside the slope zone."""
    h_runner = 2.5
    plate_lower_thk = 0.35
    runner_depth = 20.0
    flat_depth = 8.0
    slope_depth = runner_depth - flat_depth
    h_k = 1.5  # > plate_lower_thk, < h_runner
    cfg = _dynbase_cfg(
        h_k=h_k,
        h_runner=h_runner,
        plate_lower_thk=plate_lower_thk,
        runner_depth=runner_depth,
        runner_flat_depth=flat_depth,
    )
    g = build_film_gate_geometry(cfg)
    pad = cfg.pad_mm
    y_short = pad + cfg.runner_short_diameter_mm / 2
    y_flat_top = y_short + flat_depth
    y_long = y_short + runner_depth
    # Expected base from inverse-mapping h(y) = h_k
    y_base_expected = y_flat_top + (h_k - h_runner) / (plate_lower_thk - h_runner) * slope_depth
    y_top = _stage_top_y_mm(g, h_k)
    assert y_top < y_long - cfg.cell_size_mm, (
        f"paint should stop before the long edge: y_top={y_top:.2f}, y_long={y_long:.2f}"
    )
    # Within ~1 cell of the analytical base position
    assert abs(y_top - y_base_expected) <= 2 * cfg.cell_size_mm, (
        f"expected y_base={y_base_expected:.2f}, got y_top={y_top:.2f}"
    )


def test_balancer_h_at_or_above_h_runner_is_skipped() -> None:
    """h_k ≥ h_runner should produce no paint (stage skipped). The cavity
    keeps its bare runner thickness profile."""
    cfg = _dynbase_cfg(h_k=2.5, h_runner=2.5)  # h_k == h_runner exactly
    g = build_film_gate_geometry(cfg)
    # No cell should carry the residual thickness 2.5 mm exclusively as
    # balancer paint — but the trapezoid flat zone already has h_runner=2.5,
    # so we check that the *width* of the 2.5 mm region matches the trapezoid
    # silhouette (no extra ▽-shaped region carved into cells where h(y)<h_k).
    # Simpler: verify that no cell in the slope zone (y > y_flat_top) carries
    # exactly h_k=2.5 (those cells' bare runner thickness is < 2.5 there).
    pad = cfg.pad_mm
    iy_idx, _ = np.indices(g.mask.shape)
    yy = (iy_idx + 0.5) * cfg.cell_size_mm
    y_short = pad + cfg.runner_short_diameter_mm / 2
    y_flat_top = y_short + cfg.runner_flat_depth_mm
    in_slope = g.mask & (yy > y_flat_top + 0.5)  # 0.5 mm inside slope zone
    # If the stage was painted, slope-zone cells inside it would carry 2.5 mm.
    # That would be wrong because h(y) < 2.5 in the slope zone.
    assert not (in_slope & np.isclose(g.thickness_mm, 2.5)).any(), (
        "h_k ≥ h_runner should be skipped, but found 2.5 mm paint in slope zone"
    )


def test_balancer_multistage_outer_stops_short_inner_extends_long() -> None:
    """In a multi-stage balancer, the inner (thinner) stage should reach
    further than the outer (thicker) stage."""
    cfg = FilmGateConfig(
        plate_w_mm=300.0,
        plate_h_mm=50.0,
        plate_thk_mm=0.35,
        ...
        cell_size_mm=0.5,
        balancer_enabled=True,
        balancer_height_mm=18.0,
        balancer_base_distance_from_gate_mm=20.0,
        balancer_base_widths_mm=(40.0, 100.0, 180.0),
        # Pick balancer thicknesses that don't collide with the plate band
        # values (0.35 / 0.55), so np.isclose() can isolate each stage paint.
        balancer_thicknesses_mm=(0.20, 0.45, 1.45),
        plate_split_height_mm=20.0,
        plate_lower_thk_mm=0.35,
        plate_upper_thk_mm=0.55,
    )
    g = build_film_gate_geometry(cfg)
    y_top_inner = _stage_top_y_mm(g, 0.20)  # ≤ plate_thk → reaches long edge
    y_top_middle = _stage_top_y_mm(g, 0.45)  # > plate_thk → mid slope
    y_top_outer = _stage_top_y_mm(g, 1.45)  # > plate_thk → earlier slope
    # Strictly monotonic: inner extends further than middle, middle further
    # than outer.
    assert y_top_inner > y_top_middle > y_top_outer
    # Inner reaches the plate long edge
    pad = cfg.pad_mm
    y_long = pad + cfg.runner_short_diameter_mm / 2 + cfg.runner_depth_mm
    assert abs(y_top_inner - y_long) <= 2 * cfg.cell_size_mm
